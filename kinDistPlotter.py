...
# var : (xtitle, ytitle)
varnames = {
    'PhiInvMass': (r'Mass of $\phi$',r'Events/GeV')
}
...
def plot(var,year,xtitle,ytitle):
    ...
    for name in ['ttbar-had','ttbar-semi','ttbarphi-signal']:
        tempFile = ROOT.TFile.Open('rootfiles/ttbarphisnapshot_{}_{}_1of1.root'.format(name, year))
        h = tempFile.Get(var)
        if name == 'ttbarphi-signal':
            ...
            if name == 'ttbar-had':
                labelsBkg.append('ttbar-had')
                colorsBkg.append('y')
            elif name == 'ttbar-semi':
                labelsBkg.append('ttbar-semi')
                colorsBkg.append('aquamarine')


    # QCD scaling to data; Not sure exactly why I need this, but I will keep it.
    hData = histosData[0]
    hQCD = histosBkg[0]
